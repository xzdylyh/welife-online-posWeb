...
class ReportTestResult(unittest.TestResult):
    ...
    def success_counter(self, value):
        """
    ...
    def get_all_result_info_tuple(self, test):
        """
        ...
        return ([self.get_testcase_property(test), self.end_time, self.status, self.case_log])
    
    @staticmethod
    def error_or_failure_text(err):
        """
    ...
    def add_test_type(self, status, case_log):
        """
    ...
    @staticmethod
    def get_testcase_property(test):
        """
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename, log_path='.'):
        """
            生成测试报告,并放在当前运行路径下
